...
professionalRoutes.post(
  ...
  async (req, res) => {
    try { 
      ...
      const linkConfirmEmail = `${process.env.URL_FRONT || 'http://127.0.0.1:5173'}/confirmationEmail?tkn=${token}`;
      try {
...
professionalRoutes.put("/confirmationEmail", userConfirmEmailJWTDTO, async (req, res) => {
  try {
    const token = req.tkn
